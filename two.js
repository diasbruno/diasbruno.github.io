  ...
  group: {
    ...
    render: function(gl, programs) {
      ...
      if (this._mask) {
        ...
        const prop = Renderer.Utils.getRendererType(this._mask._renderer.type);
  ...
  path: {
    updateCanvas: function(gl, elem) {
      ...
      if (fill) {
        if (typeof fill === "string") {
          ...
          const prop = Renderer.Utils.getRendererType(fill._renderer.type);
      ...
      if (stroke) {
        if (typeof stroke === "string") {
          ...
          const prop = Renderer.Utils.getRendererType(stroke._renderer.type);
    ...
    render: function(gl, programs, forcedParent) {
      ...
      if (this._mask) {
        ...
        gl.stencilOp(gl.KEEP, gl.KEEP, gl.REPLACE);
        gl.colorMask(false, false, false, false);
        const prop2 = Renderer.Utils.getRendererType(this._mask._renderer.type);
...
var Utils = _.extend({
  ...
  read,
  xhr
}, _, CanvasPolyfill, curves_exports, math_exports);
var _Two = class {
  constructor(options) {
